...
public class AiQuizGenerationServiceTest {
    ...
    private final AiQuizGenerationService aiQuizGenerationService = new AiQuizGenerationService(ollamaService,docProcAiService, quizMapper, aiQuestionMapper);
    ...
    @Test
    void cgreateFromAiResponseTest() throws IOException, InterruptedException {
        ...
        // start test here
        Optional<Quiz> quiz = aiQuizGenerationService.createFromAiResponse(res);
        assert quiz.isPresent();
        assert quiz.get().getQuestionPool().size() == 20;
    }
}
